 ...
 *
 */
public class StudyOperation extends DomainOperation {
	private static final Logger log = Logger.getLogger(StudyOperation.class);
	
	private Patient patient;

	
	public StudyOperation(IDataAccess ida) {
		this.ida = ida;
	}
	...
	public Object validate(Map numbers) throws Exception {
	    Study study = null;
	    try {   				    
		    List rs = (List)ida.search(buildQueryToFindExistingStudy(numbers, patient));
		    ...
		    if (rs.size() > 1){
		    	throw new Exception ("Study table has duplicate records, please contact Data Team to fix data, then upload data again");
		    }
		    else {
		    	study = new Study();

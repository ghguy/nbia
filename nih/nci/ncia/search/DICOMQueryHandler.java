/**
 * $Id: DICOMQueryHandler.java 11967 2010-01-23 14:41:43Z kascice $
 *
 ...
 * @author NCIA Team
 */
public class DICOMQueryHandler {
     ...
     */
    private static final String SQL_QUERY_SELECT = "SELECT p.id || '/' || study.id || '/' || series.id ";
     ...
     * no results found....
     */
    public List<PatientStudySeriesTriple> findTriples(DICOMQuery theQuery) throws Exception {
        query = theQuery;
        ...
        ida = (new DataAccessProxy()).getInstance(DataAccessProxy.HIBERNATE3);
        /* List to return */
        ...
        try{
	        ...
	        List<String> results = ida.search(hql);
        	...
        	e.printStackTrace();
        	logger.error("Exception in DICOMQueryQueryHandler: " + e);
        }
        finally {
        	if (ida != null) {
        		try {
    ...
    private static String processSeriesDescriptionCriteria(DICOMQuery theQuery) throws Exception {
    ...
    private static String processMinimumStudiesCriteria(DICOMQuery theQuery) throws Exception {
     ...
     */
    private static String processAnnotationCriteria(DICOMQuery theQuery) throws Exception {
    ...
    private static String processAnatomicalSiteCriteria(DICOMQuery theQuery) throws Exception {
    ...
    private static String processAuthorizationCollections(DICOMQuery theQuery) throws Exception {
    ...
    private static String processAuthorizationSites(DICOMQuery theQuery) throws Exception {
        AuthorizationCriteria authCrit = theQuery.getAuthorizationCriteria();
    ...
    private static String processAuthorizationSecurityGroups(DICOMQuery theQuery) throws Exception {
        AuthorizationCriteria authCrit = theQuery.getAuthorizationCriteria();
     ...
     */
    private static void authorizationCriteriaPreconditions(DICOMQuery theQuery) throws Exception {
        AuthorizationCriteria authCrit = theQuery.getAuthorizationCriteria();

        // Check to ensure authorization criteria is valid
        if ((authCrit == null) || authCrit.isEmpty()) {
            throw new Exception("Authorization criteria must be included with the query");
        }

        if ((authCrit.getCollections() == null) && (theQuery.getCollectionCriteria() == null)) {
            throw new Exception("Collections must either be included in collection criteria or authorization criteria");
        }

        if ((authCrit.getCollections() != null) && (theQuery.getCollectionCriteria() != null) && 
      		!authCrit.isEmpty() && !theQuery.getCollectionCriteria().isEmpty()) {
            throw new Exception("Collections must be in either collection criteria or authorization, but not both");
        }
    }    
    ...
    private static String processCurationStatusDateCriteria(DICOMQuery theQuery) throws Exception {
    	String imgWhere = "";
        ...
        if (csdc != null) {
            ...
            imgWhere += NCIAConfig.getDateFormat().format(csdc.getCurationStatusDate());
            imgWhere += "' ";
        }
        return imgWhere;
    }
     ...
     */
    private static String processContrastAgent(DICOMQuery theQuery) throws Exception {
        ...
        if (cac != null) {
            ...
            if (contrastAgentValue.equalsIgnoreCase(ContrastAgentCriteria.UNENHANCED)) {
                ...
                throw new Exception("Not supported constrast agent criteria: " + contrastAgentValue);
            }
        }
        return "";
    }    
     ...
     */    
    private static String processDateRange(DICOMQuery theQuery) throws Exception {
        ...
        if ((drc != null) && !(drc.isEmpty())) {
            ...
            //this is MYSQL specific (to_days).  used to be Oracle specific
            //with trunc(date,DDD).  not sure there is a cross platform HQL
            //way to do date comparisons?
